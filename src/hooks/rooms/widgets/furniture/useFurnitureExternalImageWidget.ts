    ...
    useRoomEngineEvent<RoomEngineTriggerWidgetEvent>(RoomEngineTriggerWidgetEvent.REQUEST_EXTERNAL_IMAGE, event =>
    {
        ...
        roomTotalImages.forEach(object =>
        {
            if (object.type !== 'external_image_wallitem_poster' && object.type !== 'external_image_wallitem_poster_small') return null;
                ...
                datas.push({ n: JSON.parse(data).n, s: JSON.parse(data).s, t: JSON.parse(data).t, u: JSON.parse(data).u, i: JSON.parse(data).i, m: JSON.parse(data).m, w: JSON.parse(data).w });
        });
