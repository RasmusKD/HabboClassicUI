...
export const CameraWidgetEditorView: FC<CameraWidgetEditorViewProps> = props =>
{
    ...
    return (
        <NitroCardView className="nitro-camera-editor no-resize" theme="cameraeditor">
            <NitroCardHeaderView headerText={ LocalizeText('camera.editor.button.text') } onCloseClick={ event => processAction('close') } />
            <Base position="absolute" className="nitro-card-header-help" onClick={ () => CreateLinkEvent('habbopages/camera') }/>
            <NitroCardContentView>
                <Grid columnCount={ 13 }>
                    <Column size={ 5 } overflow="hidden">
                        <HabboClassicCardTabsView>
                            { TABS.map(tab =>
                            {
                        ...
                        <Column className="camera-effect-bg" overflow="hidden" gap={ 1 } >
