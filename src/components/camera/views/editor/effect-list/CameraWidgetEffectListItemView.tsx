...
import { Button, HabboClassicGridCamera, Text } from '../../../../../common';
...
export const CameraWidgetEffectListItemView: FC<CameraWidgetEffectListItemViewProps> = props =>
{
    ...
    return (
        <HabboClassicGridCamera title={ LocalizeText(!isLocked ? (`camera.effect.name.${ effect.name }`) : `camera.effect.required.level ${ effect.minLevel }`) } itemActive={ isActive } onClick={ event => (!isActive && selectEffect()) }>
            { isActive &&
