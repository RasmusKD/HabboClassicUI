        ...
        <Column gap={ 0 } overflow="hidden">
            <Column gap={ 2 }>
                <Grid gap={ 1 } className="text-black fw-bold border-bottom pb-1">
                    ...
                    <Base className="g-col-3">Rum/Spiller</Base>
                    <Base className="g-col-4">Åbnet</Base>
                ...
                { openIssues && (openIssues.length > 0) && openIssues.map(issue =>
                {
                    return (
                        <Grid key={ issue.issueId } gap={ 1 } alignItems="center" className="text-black py-1 border-bottom">
                            ...
                            <Base className="g-col-4">{ new Date(Date.now() - issue.issueAgeInMilliseconds).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: false}).replaceAll('.', ':').replaceAll(' ', ':')}</Base>
                            <Base className="g-col-3">
                                <Button variant="success" onClick={ event => SendMessageComposer(new PickIssuesMessageComposer([ issue.issueId ], false, 0, 'pick issue button')) }>Tag sagen</Button>
                            </Base>
                        </Grid>
                    );
                }) }
