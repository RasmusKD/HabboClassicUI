...
export const ModToolsView: FC<{}> = props =>
{
    ...
    return (
        <>
            { (isVisible && isMod) &&
                <Flex gap={ 0 } center className="nitro-mod-tools p-3">
                    <Column className="nitro-mod-tools-gap">
                        <ButtonModtool gap={ 1 } onClick={ event => CreateLinkEvent(`mod-tools/toggle-room-info/${ currentRoomId }`) } disabled={ (currentRoomId <= 0) } className="position-relative">
                            <Base pointer className="icon icon-small-room position-absolute start-1"/>
                        </ButtonModtool>
                        <ButtonModtool innerRef={ elementRef } gap={ 1 } onClick={ event => CreateLinkEvent(`mod-tools/toggle-room-chatlog/${ currentRoomId }`) } disabled={ (currentRoomId <= 0) } className="position-relative">
                            <Base pointer className="icon icon-chat-history position-absolute start-1"/>
                        </ButtonModtool>
                        <ButtonModtool gap={ 1 } onClick={ () => CreateLinkEvent(`mod-tools/toggle-user-info/${ selectedUser.userId }`) } disabled={ !selectedUser } className="position-relative">
                            <Base pointer className="icon icon-user position-absolute start-1"/>
                        </ButtonModtool>
                        <ButtonModtool gap={ 1 } onClick={ () => setIsTicketsVisible(prevValue => !prevValue) } className="position-relative">
                            <Base pointer className="icon icon-tickets position-absolute start-1"/>
                        </ButtonModtool>
                    </Column>
                </Flex> }
