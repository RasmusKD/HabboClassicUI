                ...
                <Flex justifyContent="between" alignItems="center" gap={ 1 }>
                    ...
                    <div className={ `form-select-profile-friends customSelect ${ isOpen ? 'active' : '' }` } onClick={ handleSelectClick } onBlur={ handleSelectBlur } tabIndex={ 0 }>
                        ...
                        <div className="options">
                            <div value="default" className={ `option ${ isOpen && order === 'default' ? 'selected' : '' }` } onClick={ () => onOrderChange('default') }>Navn A-Å</div>
                            <div value="relation" className={ `option ${ isOpen && order === 'relation' ? 'selected' : '' }` } onClick={ () => onOrderChange('relation') }>Efter Forhold</div>
                        </div>
                    </div>
                </Flex>
                        ...
                        <Flex overflow="hidden" className="profile-friends-item bg-white">
                            ...
                            <Column className="group-text" gap={ 0 }>
                                ...
                                { canRequestFriend(friendInfo._friendId) &&
                                    <Text className="request-friend-text cursor-pointer" onClick={ event => 
                                    {
                                        requestFriend(friendInfo._friendId, friendInfo._friendName);
                                    } }>{ LocalizeText('extendedprofile.addasafriend') }</Text> }
                            </Column>
                        </Flex>) }
                </Grid>
