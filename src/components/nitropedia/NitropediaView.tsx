...
export const NitropediaView: FC<{}> = props =>
{
    ...
    return (
        <NitroCardView className="nitropedia" theme="primary3" style={ dimensions ? { width: dimensions.width, height: dimensions.height } : {} }>
            <NitroCardHeaderView headerText={ header } onCloseClick={ () => setContent(null) }/>
            <NitroCardContentView>
                <Base fit innerRef={ elementRef } className="nitropedia-bg text-black" dangerouslySetInnerHTML={ { __html: content } } />
            </NitroCardContentView>
        </NitroCardView>
    );
}
