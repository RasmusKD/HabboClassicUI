...
export const NavigatorDoorStateView: FC<{}> = props =>
{
    ...
    return (
        <NitroCardView className="nitro-navigator-doorbell no-resize" windowPosition={ DraggableWindowPosition.TOP_LEFT } theme="primary">
                ...
                { !isDoorbell &&
                    <>
                        <Flex className="pt-3 align-items-center" gap={ 1 }>
                            ...
                            <input spellCheck="false" type="text" className="form-control form-control-sm password-font" onChange={ event => setPassword(event.target.value) } />
                        </Flex>
                        <Flex fullWidth gap={ 1 } className="align-items-end mt-auto pt-3">
                            <Text small className="cursor-pointer" underline onClick={ onClose }>
                                { LocalizeText('generic.cancel') }
