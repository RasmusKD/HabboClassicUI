...
const STICKIE_COLORS = [ '9CCEFF', 'FF9CFF', '9CFF9C', 'FFFF33', 'FF9C9D', 'FFCD9C', 'C3B1E1', 'DBDEFB', 'FFFFFF', '282828' ];
const STICKIE_COLOR_NAMES = [ 'blue', 'pink', 'green', 'yellow', 'red', 'orange', 'purple', 'lightblue', 'white', 'black' ];
        ...
        <DraggableWindow handleSelector=".drag-handler" windowPosition={ DraggableWindowPosition.TOP_LEFT }>
            <div className={ 'nitro-stickie nitro-stickie-image stickie-' + (type == 'post_it' ? getStickieColorName(color) : getStickieTypeName(type)) }>
                ...
                <div className="stickie-context">
                    { (!isEditing || !canModify) ? <div className="context-text" onClick={ event => (canModify && setIsEditing(true)) }>{ text }</div> : <textarea spellCheck="false" className="context-text" defaultValue={ text } tabIndex={ 0 } onBlur={ event => updateText(event.target.value) } autoFocus></textarea> }
                </div>}
                { (color === '282828') &&
                <div className="stickie-context2">
                    { (!isEditing || !canModify) ? <div className="context-text2" onClick={ event => (canModify && setIsEditing(true)) }>{ text }</div> : <textarea spellCheck="false" className="context-text" defaultValue={ text } tabIndex={ 0 } onBlur={ event => updateText(event.target.value) } autoFocus></textarea> }
                </div>}
            </div>
        </DraggableWindow>
    );
