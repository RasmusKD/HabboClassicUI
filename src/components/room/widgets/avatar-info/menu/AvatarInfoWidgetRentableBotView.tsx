        ...
        <ContextMenuView objectId={ avatarInfo.roomIndex } category={ RoomObjectCategory.UNIT } userType={ RoomObjectType.RENTABLE_BOT } onClose={ onClose } collapsable={ true }>
            ...
            { (mode === MODE_CHANGE_NAME) &&
                <Column className="menu-item context-input-bg" onClick={ null } gap={ 1 }>
                    ...
                    <input spellCheck="false" type="text" className="form-control form-control-sm" value={ newName } maxLength={ GetConfiguration<number>('bot.name.max.length', 15) } onChange={ event => setNewName(event.target.value) } />
            ...
            { (mode === MODE_CHANGE_MOTTO) &&
                <Column className="menu-item context-input-bg" onClick={ null } gap={ 1 }>
                    ...
                    <input spellCheck="false" type="text" className="form-control form-control-sm" value={ newMotto } maxLength={ GetConfiguration<number>('motto.max.length', 38) } onChange={ event => setNewMotto(event.target.value) } />
                    <Flex alignItems="center" justifyContent="between" gap={ 1 }>
                        <Button fullWidth variant="secondary" onClick={ event => processAction(null) }>{ LocalizeText('cancel') }</Button>
