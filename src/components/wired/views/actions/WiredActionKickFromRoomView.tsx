    ...
    return (
        <WiredActionBaseView requiresFurni={ WiredFurniType.STUFF_SELECTION_OPTION_NONE } hasSpecialInput={ true } save={ save }>
            <Column gap={ 1 }>
                ...
                <input spellCheck="false" type="text" className="form-control wired-form" value={ message } onChange={ event => setMessage(event.target.value) } maxLength={ GetConfiguration<number>('wired.action.kick.from.room.max.length', 100) } />
                <hr className="m-0 color-dark" />
            </Column>
        </WiredActionBaseView>
    );
