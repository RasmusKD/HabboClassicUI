...
import { LayoutFurniCountView } from './LayoutFurniCountView';
import { LayoutLimitedEditionStyledNumberView } from './limited-edition';

export interface LayoutGridFurniProps extends ColumnProps
{
    itemImage?: string;
    itemColor?: string;
    itemActive?: boolean;
...
export const LayoutGridFurni: FC<LayoutGridFurniProps> = props =>
{
    ...
    {
        const newClassNames: string[] = [ 'layout-grid-furni', 'border', 'border-1', 'border-black', 'rounded' ];
    ...
    return (
        <Column center={ center } pointer position={ position } overflow={ overflow } column={ column } classNames={ getClassNames } style={ getStyle } { ...rest }>
            { (itemCount > itemCountMinimum) &&
                <LayoutFurniCountView count={ itemCount } /> }
            { (itemUniqueNumber > 0) &&
                <>
